...
struct ResidualErrorFunctor_Rotation_Center
{
	ResidualErrorFunctor_Rotation_Center(const double* const pos_2dpoint)
	{
		m_pos_2dpoint[0] = pos_2dpoint[0];
		m_pos_2dpoint[1] = pos_2dpoint[1];
	}
	...
	*/
		...
		T* out_residuals) const
	{
		...
		pos_proj[0] = pos_3dpoint[0] - center[0];
		pos_proj[1] = pos_3dpoint[1] - center[1];
		pos_proj[2] = pos_3dpoint[2] - center[2];

		// Rotate the point according the camera rotation
		ceres::AngleAxisRotatePoint(rotation, pos_proj, pos_proj);
